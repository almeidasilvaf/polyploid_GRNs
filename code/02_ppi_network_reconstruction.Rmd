...
# Define function to create PPI network of paralogs from STRINGdb for a species
stringdb2paralogsppi <- function(species_id, threshold = 500, 
...
# Get PPI network for paralogous gene pairs
ppi <- lapply(id_map, function(x) {
    paralogs_ppi <- stringdb2paralogsppi(
        x, 
        id_map = id_map, 
...
fisher_dup_interaction <- function(duplicated_genes, ppi) {
...
dup_ppi_association <- fisher_dup_interaction(
    duplicated_genes, ppi
)
dup_ppi_association
```

As we can see, the PPI network of *Glycine max*, *P. trichocarpa* and *Z. mays*
is enriched in WGD-derived genes. That means that WGD-derived genes
